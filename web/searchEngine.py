...
class SearchEngine:
    ...
    def __init__(self, config_path, config_encoding):
        ...
        f = io.open(config['DEFAULT']['stop_words_path'], encoding = config['DEFAULT']['stop_words_encoding'])
        words = f.read()
        ...
        for term in cleaned_dict.keys():
            ...
            for doc in docs:
                ...
                if docid in time_scores:
                    continue
                #tss1 = '2019-06-02 21:19:03'
                # 转为时间数组
                #timeArray = time.strptime(tss1, "%Y-%m-%d %H:%M:%S")
                #print(timeArray)
                #print(time.strptime("%Y", date_time))
                #print(time.strptime(date_time,"%Y年%m月%d日 %H:%M"))
                #print(datetime.timetuple())
                #print(datetime.strftime("%a %b %d %H:%M:%S %Y"))
                news_datetime = time.strptime(date_time,"%Y年%m月%d日 %H:%M")
                now_datetime = time.localtime(time.time())
                td = time.mktime(now_datetime) - time.mktime(news_datetime)
                docid = int(docid)
                td = (td / 3600) # hour
